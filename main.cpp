...
const sf::Vector2f firstFloorCoordinates = sf::Vector2f(
    SCREEN_WIDTH / 2,
    SCREEN_HEIGHT -
        70); // coordinates relative to the middle of 300x170 elevator
const sf::Vector2f secondFloorCoordinates =
    sf::Vector2f(SCREEN_WIDTH / 2, SCREEN_HEIGHT - (3 * 70));
const sf::Vector2f thirdFloorCoordinates =
    sf::Vector2f(SCREEN_WIDTH / 2, SCREEN_HEIGHT - (5 * 70));
const sf::Vector2f fourthFloorCoordinates =
    sf::Vector2f(SCREEN_WIDTH / 2, SCREEN_HEIGHT - (7 * 70));
const sf::Vector2f fifthFloorCoordinates =
    sf::Vector2f(SCREEN_WIDTH / 2, SCREEN_HEIGHT - (9 * 70));
...
public:
  Button(float x, float y, float width, float height, sf::Font *font,
    ...
    m_textPositionVector =
        sf::Vector2f((m_shape.getGlobalBounds().getPosition().x) +
                         (m_shape.getGlobalBounds().getSize().x / 2.f) -
                         (m_text.getGlobalBounds().getSize().x / 2.f),
                     (m_shape.getGlobalBounds().getPosition().y) +
                         (m_shape.getGlobalBounds().getSize().y / 2.f) -
  ...
  bool clicked(sf::Event evnt, sf::Vector2i mousePossition) {
    if (mousePossition.x <= m_shape.getPosition().x + m_shape.getSize().x &&
        mousePossition.x >= m_shape.getPosition().x &&
        mousePossition.y <= m_shape.getPosition().y + m_shape.getSize().y &&
        mousePossition.y >= m_shape.getPosition().y) {
      std::cout << "Kupa";
      return true;
    }
    return false;
...
public:
  Passenger(const sf::Texture *texture, int startLevel, int endLevel) {
    ...
    switch (startLevel) {
    case 0:
      this->mSprite.setPosition(
          sf::Vector2f(300.f, firstFloorCoordinates.y + 70));
      break;
    case 1:
      this->mSprite.setPosition(
          sf::Vector2f(500.f, secondFloorCoordinates.y + 70));
      break;
    case 2:
      this->mSprite.setPosition(
          sf::Vector2f(300.f, thirdFloorCoordinates.y + 70));
      break;
    case 3:
      this->mSprite.setPosition(
          sf::Vector2f(500.f, fourthFloorCoordinates.y + 70));
      break;
    case 4:
      this->mSprite.setPosition(
          sf::Vector2f(300.f, fifthFloorCoordinates.y + 70));
      break;
    }
  ...
  int getStartLevel() { return mStartLevel; }

  int getEndLevel() { return mEndLevel; }
...
public:
  Floor(int id, sf::Font *font) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      Button button((100.f + (m_id % 2) * 900.f + i * 40),
                    50.f + static_cast<int>(m_id) * 125.f, 30.f, 30.f, font, i);
  ...
  void listenForButtons(sf::Event evnt, sf::Vector2i mousePos,
                        const sf::Texture *texture) {
    for (int i = 0; i < 4; i++) {
      if (m_otherFloorsButtons[i].clicked(evnt, mousePos)) {
        Passenger newPassenger(texture, m_id,
                               m_otherFloorsButtons[i].getValue());
...
int main() {
  ...
  while (window.isOpen()) {
    ...
    for (int i = 0; i < 5; i++) {
      floors[i].listenForButtons(event, sf::Mouse::getPosition(), &texture);
    }
